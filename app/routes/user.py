...
def create_user_router() -> APIRouter:
    ...
    @user_router.get("/user/me", response_model=Profile)
    async def get_user():
        user_profile = await user_service.get_user_info()

        return user_profile


    @user_router.get("/user/{user_id}", response_model=Profile)
    ...
    @user_router.get("/users", response_model=ManyUserResponse)
    ...
    @user_router.post("/users", response_model=CreateUserRes)
